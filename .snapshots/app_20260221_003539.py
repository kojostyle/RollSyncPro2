...
def minutes_to_display(minutes_value):
    total_seconds = max(0, int(round(minutes_value * 60)))
    mins = total_seconds // 60
    secs = total_seconds % 60
    return mins + (secs / 100.0)

def display_to_minutes(display_value):
    mins = int(display_value)
    secs = int(round((display_value - mins) * 100))
    mins += secs // 60
    secs = secs % 60
    return max(0.0, mins + (secs / 60.0))

if "run_time_display" not in st.session_state:
    st.session_state.run_time_display = minutes_to_display(st.session_state.run_time_min)

def sync_run_time_from_display():
    st.session_state.run_time_min = display_to_minutes(st.session_state.run_time_display)
    st.session_state.run_time_display = minutes_to_display(st.session_state.run_time_min)

def adjust_run_time(seconds_delta):
    st.session_state.run_time_min = max(
        0.0, st.session_state.run_time_min + (seconds_delta / 60.0)
    )
    st.session_state.run_time_display = minutes_to_display(st.session_state.run_time_min)
...
with col3:
    run_label_col, run_input_col = st.columns([1.3,2.0])
    with run_label_col:
        st.markdown("<div style='margin-top:6px'>運転時間 (min)</div>", unsafe_allow_html=True)
    with run_input_col:
        run_val_col, run_m_col, run_p_col = st.columns([2.2,0.8,0.8])
        with run_val_col:
            st.number_input(
                "運転時間 (min)",
                min_value=0.0,
                step=0.01,
                format="%.2f",
                key="run_time_display",
                value=st.session_state.run_time_display,
                label_visibility="collapsed",
                on_change=sync_run_time_from_display
            )
        with run_m_col:
            st.button("➖", key="run_time_minus", on_click=adjust_run_time, args=(-1,))
        with run_p_col:
            st.button("➕", key="run_time_plus", on_click=adjust_run_time, args=(1,))
