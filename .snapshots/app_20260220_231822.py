...
.big-speed {font-size:26px; font-weight:bold;}
.big-diff {font-size:30px; font-weight:bold;}
...
# 🏭 ロール制御
# ==============================
title_col, r_label_col, r_input_col = st.columns([6,0.7,0.8])
with title_col:
...
for i in range(R):
    ...
    radius = 0.2 + 0.3*(surface_speeds[i]/1300 if belt_speed!=0 else 0)
    circle = plt.Circle((i*spacing,0), radius, fill=False, linewidth=3, color=color)
    ax.add_patch(circle)
ax.set_xlim(-0.5,R*spacing)
ax.set_ylim(-1,1)
ax.set_aspect("equal")
ax.axis("off")
st.pyplot(fig)
